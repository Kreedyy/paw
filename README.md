# ![Logo: a terminal with a paw shaped prompt](icons/hicolor/48x48/apps/paw.png) paw

The fast, lightweight and minimalistic Wayland terminal emulator.

[![CI status](https://ci.codeberg.org/api/badges/dnkl/paw/status.svg)](https://ci.codeberg.org/dnkl/paw)

[![Packaging status](https://repology.org/badge/vertical-allrepos/paw.svg?columns=4)](https://repology.org/project/paw/versions)
...
1. [Why the name 'paw'?](#why-the-name-paw)
1. [Fonts](#fonts)
...
1. [Programmatically checking if running in paw](#programmatically-checking-if-running-in-paw)
...
* Fast (see [benchmarks](doc/benchmark.md), and
  [performance](https://codeberg.org/dnkl/paw/wiki/Performance))
...
* [Sixel image support](https://en.wikipedia.org/wiki/Sixel)
  ...
  ![tux-with-paw](doc/sixel-tux-paw.png "Sixel screenshot")
...
**paw** can be configured by creating a file
`$XDG_CONFIG_HOME/paw/paw.ini` (defaulting to
`~/.config/paw/paw.ini`).  A template for that can usually be found
in `/etc/xdg/paw/paw.ini` or
[here](https://codeberg.org/dnkl/paw/src/branch/master/paw.ini).

Further information can be found in paw's man page `paw.ini(5)`.


## Troubleshooting

See the [wiki](https://codeberg.org/dnkl/paw/wiki#user-content-troubleshooting)
...
_foo terminal_ → _pawerm_ → _paw_

Pretty bad, I know.

As a side note, if you pronounce the _foo_ part of _paw_ the same way
you pronounce _foobar_, then _paw_ sounds a lot like the Swedish word
_fot_, which incidentally means (you guessed it) _paw_.


## Fonts

**paw** supports all fonts that can be loaded by _freetype_,
including **bitmap** fonts and **color emoji** fonts.

Paw uses _fontconfig_ to locate and configure the font(s) to
...
requested font is one), paw allows you, the user, to configure the
...
These are the default shortcuts. See `man paw.ini` and the example
`paw.ini` to see how these can be changed.
...
: Spawn a new terminal. If the shell has been [configured to emit the
  OSC 7 escape
  sequence](https://codeberg.org/dnkl/paw/wiki#user-content-spawning-new-terminal-instances-in-the-current-working-directory),
...
: Jump to the previous, currently not visible, prompt. Requires [shell
  integration](https://codeberg.org/dnkl/paw/wiki#user-content-jumping-between-prompts).
...
When run normally, **paw** is a single-window application; if you
want another window, start another paw process.

However, paw can also be run in a _server_ mode. In this mode, one
process hosts multiple windows. All Wayland communication, VT parsing
and rendering is done in the server process.

New windows are opened by running `pawclient`, which remains running
until the terminal window is closed, at which point it exits with the
exit value of the client process (typically the shell).

The point of this mode is **a)** reduced memory pawprint - all
...
Typical usage would be to start the server process (`paw --server`)
when starting your Wayland compositor (i.e. logging in to your
desktop), and then run `pawclient` instead of `paw` whenever you
want to launch a new terminal.

Paw supports socket activation, which means `paw --server` will only be
started the first time you'll run `pawclient`. (systemd user units are
included, but it can work with other supervision suites).

## URLs

Paw supports URL detection. But, unlike many other terminal
emulators, where URLs are highlighted when they are hovered and opened
by clicking on them, paw uses a keyboard driven approach.
...
bindings in paw. See `show-urls-launch` and `show-urls-copy` in the
`paw.ini` man page.
...
New paw terminal instances (bound to
...
[wiki](https://codeberg.org/dnkl/paw/wiki#user-content-spawning-new-terminal-instances-in-the-current-working-directory)
for details.


### Jumping between prompts

Paw can move the current viewport to focus prompts of already
...
See the
[wiki](https://codeberg.org/dnkl/paw/wiki#user-content-jumping-between-prompts)
...
```ini
[key-bindings]
pipe-command-output=[sh -c "f=$(mktemp); cat - > $f; pawclient emacsclient -nw $f; rm $f"] Control+Shift+g
```
...
is started in a new pawclient instance. The temporary file is removed
after the pawclient instance has closed.
...
```fish
function paw_cmd_start --on-event fish_preexec
  echo -en "\e]133;C\e\\"
end

function paw_cmd_end --on-event fish_postexec
  echo -en "\e]133;D\e\\"
end
```

See the
[wiki](https://codeberg.org/dnkl/paw/wiki#user-content-piping-last-command-s-output)
for details, and examples for other shells


## Alt/meta

By default, paw prefixes _Meta characters_ with ESC. This corresponds
to XTerm's `metaSendsEscape` option set to `true`.

This can be disabled programmatically with `\E[?1036l` (and enabled
again with `\E[?1036h`).

When disabled, paw will instead set the 8:th bit of meta character
...
Paw transmits DEL (`^?`) on <kbd>backspace</kbd>. This corresponds to
...
Note that paw does **not** implement `DECBKM`, and that the behavior
...
are expected to behave, paw >= 1.15 will default to scaling fonts
using the compositor's scaling factor, and **not** the monitor
DPI.

This means the (assuming the monitors are at the same viewing
distance) the font size will appear to change when you move the paw
...
`paw.ini`. When enabled, fonts will **not** be sized using the
scaling factor, but will instead be sized using the monitor's
DPI. When the paw window is moved across monitors, the font size is
...
with the terminal emulator itself. Paw implements the following OSCs:
...
* `OSC 176` - set app ID
* `OSC 555` - flash screen (**paw specific**)
* `OSC 777` - desktop notification (only the `;notify` sub-command of
  OSC 777 is supported.)

See the **paw-ctlseqs**(7) man page for a complete list of supported
control sequences.


## Programmatically checking if running in paw

Paw does **not** set any environment variables that can be used to
identify paw (reading `TERM` is not reliable since the user may have
...
`464f4f54` is `PAW` in hex.

The secondary DA response is `\E[>1;XXYYZZ;0c`, where `XXYYZZ` is
paw's major, minor and patch version numbers, in decimal, using two
digits for each number. For example, paw-1.4.2 would respond with
`\E[>1;010402;0c`.
...
parse the response (which in paw will consist of all three DA
responses, all at once) to determine which requests the terminal
emulator actually responded to.

Starting with version 1.7.0, paw also implements `XTVERSION`, to
which it will reply with `\EP>|paw(version)\E\\`. Version is
...
paw's terminfo on remote hosts you SSH into.
...
Paw supports this, and extends it even further, to also include
boolean capabilities. This means paw's entire terminfo can be queried
via `XTGETTCAP`.

Note that both Kitty and paw handles **responses** to
...
Kitty and paw on the other hand, send one DCS response for **each**
...
capability name, paw's reply is identical to what `tigetstr()` would
have returned.
...
where `$(uname -s)` is the name of the OS paw was compiled for.


# Credits

* [Ordoviz](https://codeberg.org/Ordoviz), for designing and
contributing paw's [logo](icons/hicolor/48x48/apps/paw.png).
...
Please report bugs to https://codeberg.org/dnkl/paw/issues
...
- Log output from paw (run `paw -d info` from another terminal).
...
Ask questions, hang out, sing praise or just say hi in the `#paw`
channel on
[irc.libera.chat](https://web.libera.chat/?channels=#paw). Logs are
available at https://libera.irclog.whitequark.org/paw.


## Mastodon

Every now and then I post paw related updates on
